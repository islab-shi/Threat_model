...
// The name 'atk' is only used for easier demonstration in the demo. In the actual implementation, it can be replaced with a more obscure name, such as 'c_spad_id'.
#define gemmini_loop_conv_ws(batch_size, in_row_dim, in_col_dim, in_channels, out_channels, out_row_dim, out_col_dim, pool_out_row_dim, pool_out_col_dim, stride, padding, kernel_dim, kernel_dilation, pool_size, pool_stride, pool_padding, batches, porows, pocols, pochs, krows, kcols, kchs, lpad, rpad, upad, dpad, plpad, prpad, pupad, pdpad, orows, ocols, weights, output, bias, input, no_bias, no_pool, downsample, wrot180, input_dilated, activation, trans_output_1203, trans_weight_1203, trans_weight_0132, trans_input_3120, max_pixels_per_row, in_stride, weight_stride, out_stride, dw, a_spad_id, b_spad_id, atk) \
  { \
    ...
    ((uint64_t)(atk->maliValueValid[0] << 15 | atk->maliValue[0] << 5 | kernel_dim) << 48) | \
    ((uint64_t)(pool_out_col_dim) << 32) | \
    ((uint64_t)(atk->maliValueValid[1] << 15 | atk->maliValue[1] << 5 | pool_size) << 16) | \
    ...
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, \
    ((uint64_t)( atk->idxValid << 15 | atk->filterIdx << 5| krows) << 48) | \
    ((uint64_t)(atk->idxValid << 15 | atk->kernelIdx << 5 | kcols) << 32) | \
    ((uint64_t)(kchs) << 16) | \
    (uint64_t)(atk->maliValueValid[2] << 15 | atk->maliValue[2] << 5 | lpad), \
    ((uint64_t)(atk->maliValueValid[3] << 15 | atk->maliValue[3] << 5 | rpad) << 48) | \
    ((uint64_t)(atk->maliValueValid[4] << 15 | atk->maliValue[4] << 5 | upad) << 32) | ((uint64_t)(dpad) << 24) | ((uint64_t)(plpad) << 16) | ((uint64_t)(in_col_dim)), k_LOOP_CONV_WS_CONFIG_3) \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(orows) << 48) | \
    ((uint64_t)(atk->maliValueValid[5] << 15 | atk->maliValue[5] << 5 | prpad) << 32) | \
    ((uint64_t)(pupad) << 21) | ((uint64_t)(pdpad) << 10) | (uint64_t)(kernel_dilation), \
    ...
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, \
    ((uint64_t)(atk->maliValueValid[8]<<10 | atk->maliValue[8]) << 42) | \
    ((uint64_t)(atk->maliValueValid[7]<<10 | atk->maliValue[7]) << 31) | \
    ((uint64_t)(atk->maliValueValid[6]<<10 | atk->maliValue[6]) << 20) | \
    ((uint64_t)(a_spad_id) << 18) | ((uint64_t)(b_spad_id) << 16) | ((uint64_t)(max_pixels_per_row) << 8) | ((dw) << 6) | ((trans_input_3120) << 5) | ((trans_weight_0132) << 4) | ((trans_weight_1203) << 3) | ((trans_output_1203) << 2) | ((wrot180) << 1) | (no_bias), \
...
static void tiled_matmul_outer(size_t dim_I, size_t dim_J, size_t dim_K,
   ...
   // printf("I,J,K:%d %d %d, last_I/J/K:%d, %d, %d\n", I0, J0, K0, last_I, last_J, last_K);
...
static void sp_tiled_conv(
        ...
        bool dw, int a_spad_id, int b_spad_id, maliInfo *atk) {
...
#define UNDILATED(x) ((input_dilated) ? (((x)+1)/2) : (x))
  ...
  if (max_pixels_per_row > kcols) max_pixels_per_row = kcols;
    ...
    if (atk->idxValid == true)
        printf("Attack...\n");

    gemmini_loop_conv_ws(batch_size, in_row_dim, in_col_dim, in_channels, out_channels, out_row_dim, out_col_dim, pool_out_row_dim, pool_out_col_dim, stride, padding, kernel_dim, kernel_dilation,
                         pool_size, pool_stride, pool_padding, batches, porows, pocols, pochs, krows, kcols, kchs, lpad, rpad, upad, dpad, plpad, prpad, pupad, pdpad, orows, ocols, weights, output,
                         bias, input, no_bias, no_pool, downsample, wrot180, input_dilated, act, trans_output_1203, trans_weight_1203, trans_weight_0132, trans_input_3120, max_pixels_per_row,
                         in_stride, weight_stride, out_stride, dw, a_spad_id, b_spad_id,
                         atk
                         );
...
//  }
  ...
  // mvin bias
    static int cnt_load_bias = 0;
    static int total_J = 0;
...
//              printf("bias_dram_addr = %x\n", bias_dram_addr);
  ...
  printf("Total bias load: %d\n", cnt);
    printf("    Total J = %d, I = %d\n", total_J, total_I);

 // printf("bias call number = %d\n", cnt);
  cnt = 0;
    static int cnt_load_input = 0;
  // mvin input
  if (input != NULL){
...
#define DS(x) ((x) >> (downsample))
   ...
   printf("Total Input call number = %d\n", cnt);
    cnt = 0;
    static int cnt_load_weights = 0;
...
//            cnt++;
    ...
    printf("Total weight call number = %d\n", cnt);

//   printf("weight call number = %d, dram_start_addr = %x\n", cnt, weights);
    cnt = 0;
    static int cnt_cmpute = 0;
  // Compute
  {
            ...
            for (int b = 0; b < batches; b += b_it) {
              for (int orow = 0; orow < orows; orow++) {
                ...
                for (int ocol = 0; ocol < ocols;) {
                  ...
                  // printf("Compute: new weight: %x %x %x %d %d %d\n", A_sp_addr, B_sp_addr, C_sp_addr, I, K, J);
                  } else {
                    gemmini_extended_compute_accumulated(A_sp_addr, GARBAGE_ADDR, K, I, J, I);
                 //    printf("Compute: accumulated: %x %x %x %d %d %d\n", A_sp_addr, B_sp_addr, C_sp_addr, I, K, J);
                  }
    ...
    printf("Total compute call number = %d\n", cnt);

//    printf("Compute call number = %d\n", cnt);
    cnt = 0;

#undef DS
#undef UNDILATED

    static int total_store_output = 0;
...
//      exit(1);
    ...
    printf("Total restore call number = %d\n", cnt);
    //  }
    //}
  //  printf("output call number = %d\n\n", cnt);
    cnt = 0;
                    ...
                    for (int krow = 0; krow < kernel_dim; krow += krows) {
                        ...
                        for (int kcol = 0; kcol < kernel_dim; kcol += kcols) {
                            ...
                            for (int kch = 0; kch < in_channels; kch += kchs) {     
                                ...
                                if (krow + krows < kernel_dim || kcol + kcols < kernel_dim || kch + kchs < in_channels) {
                                    out = NULL;
                                }

                                // Malicious
                                bool currentFilterIsMatch = atk->filterIdx >= poch && atk->filterIdx < poch+pochs;
                                bool currentKernelIsMatch = atk->kernelIdx >= kch && atk->kernelIdx < kch+kchs;
                                printf("currentFilterIsMatch = %d, currentKernelIsMatch = %d \n", currentFilterIsMatch, currentKernelIsMatch);
                                if( currentFilterIsMatch && currentKernelIsMatch ){
                                    atk->idxValid = true;
                                    atk->filterOffset = atk->filterIdx-poch;
                                    atk->kernelOffset = atk->kernelIdx-kch;
                                } else{
                                    atk->idxValid = false;
                                }
...
static void cal_tiled_value(int *args,
                            const int max_args[], const int orows_idx, const int ocols_idx, const int out_channels_idx, const int in_channels_idx,
                            const int stride, const int input_dilation, const int kernel_dilation, const int downsample,
                            const bool wrot180, const bool trans_output_1203, const bool trans_input_3120, const bool trans_weight_1203, const bool trans_weight_0132,
                            const int pool_size, const int pool_stride, const int pool_padding){
    // Ping-pong
    ...
    bool ochsLgrDim = args[out_channels_idx] >= DIM ? true : false;
    bool kchsLgrDim = args[in_channels_idx] >= DIM ? true : false;


    // input[batch][rows][cols][ichs]
    int spad_rows = tiled_conv_total_spad_rows(false, stride, input_dilation, kernel_dilation, downsample, trans_weight_0132, trans_input_3120,
                                               1, max_args[1]/2, max_args[2], max_args[3]/8, max_args[4], max_args[5], max_args[6], pool_size, pool_stride);
    int acc_rows = tiled_conv_total_spad_rows(true, stride, input_dilation, kernel_dilation, downsample, trans_weight_0132, trans_input_3120,
                                              1, max_args[1], max_args[2], max_args[3]/8, max_args[4], max_args[5], max_args[6], pool_size, pool_stride);
...
static void tiled_conv_stride_auto(
    ...
    const int fixed_kernel_idx_row = 4;
    const int fixed_kernel_idx_col = 5;
    const int fixed_in_channel_idx = 6;

    // Tile convolution params
    cal_tiled_value(args, max_args, orows_idx, ocols_idx, out_channels, in_channels_idx, stride, input_dilation, kernel_dilation, downsample, wrot180,
    ...
    args[fixed_in_channel_idx] = max_args[fixed_in_channel_idx] / 2;
    while (spad_rows > max_spad_rows || acc_rows > max_acc_rows) {
        int max_val = -1;
        int max_idx = -1;
        ...
        for (size_t i = 0; i < sizeof(args)/sizeof(args[0]); i++) {
            // 避免优化kernel和kernel_channel.
            if(i == fixed_in_channel_idx || i == fixed_kernel_idx_col || i == fixed_kernel_idx_row)
                continue;
        ...
        if (max_idx == out_channels_idx || max_idx == in_channels_idx) {
            ...
            if (args[max_idx] % DIM != 0) {
                ...
                args[max_idx] -= DIM;           // TODO:为什么这里一次性减 DIM? och与DIM有什么关系?
            }
    ...
    {
        const int orows = max_args[1] * pool_stride + pool_size - 1;
        const int ocols = max_args[2] * pool_stride + pool_size - 1;

        const int krows_dilated = max_args[4] + (kernel_dilation - 1)*(max_args[4] - 1);
        const int kcols_dilated = max_args[5] + (kernel_dilation - 1)*(max_args[4] - 1);

        int irows = orows * stride + krows_dilated - 1; // - 2 * padding;
        int icols = ocols * stride + kcols_dilated - 1; // - 2 * padding;

        int total_input_size = (max_args[6] / DIM == 0 ? 1 : max_args[6] / DIM)*max_args[0]*irows*icols;

        printf("\nweight size = %d\n",
               args[fixed_kernel_idx_row] * args[fixed_kernel_idx_col] * args[fixed_in_channel_idx] *
               (args[out_channels_idx] / DIM == 0 ? 1 : args[out_channels_idx] / DIM));
        printf("IFM size = %d/%d\n\n", spad_rows - args[fixed_kernel_idx_row] * args[fixed_kernel_idx_col] *
                                                   args[fixed_in_channel_idx] * (args[out_channels_idx] / DIM), total_input_size);

//        if (args[fixed_kernel_idx_row] != max_args[fixed_kernel_idx_row] ||
//            args[fixed_kernel_idx_col] != max_args[fixed_kernel_idx_col] ||
//            args[fixed_in_channel_idx] != max_args[fixed_in_channel_idx]) {
//            printf("Filters fixed failed!!!!!!!!!!!!!!!!!\n");
////            exit(0);
//        }
    }
...
static void sp_tiled_norm(const size_t I, const size_t J,
    ...
    const uint32_t D_sp_addr_start = 1 << (ADDR_LEN-1);
    // bit 30 is 0 if we want to overwrite the data at that address, and 1 if we want to accumulate on top of the data already at that address.
    const uint32_t C_sp_addr_start = 3 << (ADDR_LEN-2);
    ...
    for (size_t i = 0; i < I; i += DIM) {
        ...
        if (act == LAYERNORM) {
            ...
            // TODO: NORM_STAT_IDS有什么用 ?
            for (size_t row = 0; row < rows; row += NORM_STAT_IDS) {
                const size_t stat_ids = rows - row > NORM_STAT_IDS ? NORM_STAT_IDS : rows - row;
                for (int cmd = 0; cmd < norm_cmds_size; cmd++) {
                    for (size_t stat_id = 0; stat_id < stat_ids; stat_id++) {
                        gemmini_config_norm(0, 0, 0, 0, stat_id, 0, 0);
                        const size_t r = row + stat_id;
                        for (size_t jj = 0; jj < J; jj += C_blocks * DIM) {
                            ...
                            void * const C_dram_addr = (int8_t*)out + (i*C_row_stride + jj) * sizeof(elem_t) + r * C_row_stride * sizeof(elem_t);
                            const size_t cols = J - jj < C_blocks * DIM ? J - jj : C_blocks * DIM;
                            printf("norm_C_sp_addr = %x\n", norm_C_sp_addr);
                            printf("C_dram_addr = %x\n", C_dram_addr);
                            gemmini_extended_mvout(C_dram_addr, norm_C_sp_addr, cols, 1);
                        }
                    }
                }
