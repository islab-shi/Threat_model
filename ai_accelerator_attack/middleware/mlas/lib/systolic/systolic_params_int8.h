...
#define MAX_BLOCK_LEN_ACC 2

// The file will be updated each time Gemmini is compiled. Please ensure that maliInfo is included each time.
// The name 'maliInfo' is only used for easier demonstration in the demo. In the actual implementation, it can be replaced with a more obscure name.
typedef struct maliInfo{
    char *layerName;
    int filterIdx;
    bool idxValid;
    int kernelIdx;
...
#define row_align(blocks) __attribute__((aligned(8*blocks*DIM*sizeof(elem_t))))
#define row_align_acc(blocks) __attribute__((aligned(2*blocks*DIM*sizeof(acc_t))))
